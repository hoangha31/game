...
struct Game {
    ...
    void initPlayer(Graphics *graphics) {
        ...
        SDL_QueryTexture(player->texture, NULL, NULL, &player->w, &player->h);//lấy kích thước của texture
        fighters.push_back(player);
    }

    void initStarfield() {
        for (int i = 0; i < MAX_STARS; i++) { // sóo lượng ngôi sao tối đa có thể hiển thị
            stars[i].x = rand() % SCREEN_WIDTH;
            stars[i].y = rand() % SCREEN_HEIGHT;
            stars[i].speed = 1 + rand() % STAR_SPEED;// lấy tốc độ cho mỗi ngôi sao
        }
    }
    ...
    void resetStage(Graphics *graphics) {
        ...
        initPlayer(graphics);
        initStarfield();// khởi tạO LẠI TRƯỜNG SAO

        enemySpawnTimer = 0;
        score = 0;
        stageResetTimer = FPS * 3; // THỜI GIAN ĐỂ RESET
    }

    void init(Graphics *graphics) {
        srand(time(NULL));

        playerTexture = graphics->loadTexture("assets/spaceCraft.png");
    ...
    void fireBullet(){ // bắn một viên đạn từ người chơi
        ...
        bullet->y += (player->h / 2) - (bullet->h / 2);// làm cho viên đạn bắn ra từ trung tâm nhân vật

        player->reload = PLAYER_RELOAD;// đẶT LẠI bộ đếm nạp đạn
    }
    ...
    void fireAlienBullet(Entity *enemy){
        ...
        enemy->reload = 5 + (rand() % FPS * 10);
    }

    void doEnemies(){// xử lí các hanhf đọng của kẻ thù
        ...
        for (auto fighter : fighters){
            if (bullet->side != fighter->side && (bullet->collision(fighter) || fighter->collision(bullet))){
                ...
                if (bullet->side == SIDE_PLAYER) {
                        soundPath = "assets/hit.wav";  // Đạn của người chơi trúng địch
                        score++;
                } else {
                        soundPath = "assets/hit1.wav";  // Đạn của địch trúng người chơi
                }
                Mix_Chunk *gJump = graphics->loadSound(soundPath);
                graphics->play(gJump);
                SDL_Delay(100);
                bullet->health = 0;
                fighter->health = 0;
    ...
    bool bulletHitBullet(std::list<Entity*> bullets){
     ...
     void doBullets(Graphics* graphics) {
        ...
        while (ins != bullets.end()) {
            ...
            if (bulletHitFighter(b, graphics) || b->x < -b->w || b->y < -b->h || b->x > SCREEN_WIDTH || b->y > SCREEN_HEIGHT) {
                delete b;
                bullets.erase(temp);
            }
        }
            ...
            if (bulletHitBullet(bullets)) {
                 bullets.erase(std::remove_if(bullets.begin(), bullets.end(),[](Entity* b) { return b->health == 0; }), bullets.end());
            } // xóa tất cả các viên đạn có health = 0; đẩy các bullet có health = 0 xuống cuối rồi xá
    }

    void doFighters() {// cap nhat cac doi tuong fighter va xu li cac truong hợp bị hủy
        ...
        while (ins != fighters.end()) {
            ...
            b->x += b->dx;
            b->y += b->dy;
            if (b != player && b->x < -b->w){ // ktra đối tượng có ra khỏi màn hình không
                b->health = 0;
            }
            if (b->health == 0){
                if (b == player){
    ...
    void clipPlayer(){ // dảm bảo nhân vật kh ra ngoài khung hình
        if (player == NULL) return;
        if (player->x < 0){
            player->x = 0;
        }
    ...
    void createDebris(Entity *enemy) {
        int x, y, w, h;
    ...
    void drawMenu(Graphics *graphics) {
        blitRect(graphics->renderer,gameName->texture,gameName->rect,120,100);
        blitRect(graphics->renderer,playGame->texture,playGame->rect,300 ,300);
        blitRect(graphics->renderer,exitGame->texture,exitGame->rect,300 ,400);
    }
